...
var { Shop, Item, AgedBrie, BackstagePasses, Sulfuras, NormalItem } = require('../src/gilded_rose.js');
...
describe("Gilded Rose", function () {
  ...
  it('sulfuras could not be sold', () => {
    const gildedRose = new Shop([new Sulfuras("Sulfuras, Hand of Ragnaros", 10, 0)]);
    const items = gildedRose.updateQuality()
    expect(items[0].sellIn).to.equal(10);
  })

  it('sulfuras could not decrease quality', () => {
    const gildedRose = new Shop([new Sulfuras("Sulfuras, Hand of Ragnaros", 10, 10)]);
    const items = gildedRose.updateQuality()
  ...
  it('item with date passed quality decrease by twice', () => {
    const gildedRose = new Shop([new NormalItem("foo", -1, 40)]);
    const items = gildedRose.updateQuality()
    expect(items[0].quality).to.equal(38);
  })

  it('aged brie increase quality when it gets older', () => {
    const gildedRose = new Shop([new AgedBrie("Aged Brie", 1, 40)]);
    const items = gildedRose.updateQuality()
  ...
  it('backstage passes increase quality by two when sellin less than ten', () => {
    const gildedRose = new Shop([new BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 10, 40)]);
    const items = gildedRose.updateQuality()
    expect(items[0].quality).to.equal(42);
  })
  ...
  it('backstage passes increase quality by three when sellin less than five', () => {
    const gildedRose = new Shop([new BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 5, 40)]);
    const items = gildedRose.updateQuality()
  ...
  it('backstage passes increase quality by two when sellin less than six and not more than fifty', () => {
    const gildedRose = new Shop([new BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 6, 49)]);
    const items = gildedRose.updateQuality()
    expect(items[0].quality).to.equal(50);
  })

  it('backstage passes increase quality by three when sellin less than five and not more than fifty', () => {
    const gildedRose = new Shop([new BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 5, 48)]);
    const items = gildedRose.updateQuality()
    expect(items[0].quality).to.equal(50);
  })

  it('backstage passes quality drops to zero after concert', () => {
    const gildedRose = new Shop([new BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 0, 40)]);
    const items = gildedRose.updateQuality()
    expect(items[0].quality).to.equal(0);
  })

  it('backstage passes quality increase quality by one when date is more than 10', () => {
    const gildedRose = new Shop([new BackstagePasses("Backstage passes to a TAFKAL80ETC concert", 11, 40)]);
    const items = gildedRose.updateQuality()
    expect(items[0].quality).to.equal(41);
  })
